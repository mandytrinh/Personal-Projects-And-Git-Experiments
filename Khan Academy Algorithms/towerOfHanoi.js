...
var solveHanoi = function(numDisks, fromPeg, toPeg) {
    ...
    // recursive case:
    // 1) move everything but the bottom most disk to the spare peg
    var sparePeg = hanoi.getSparePeg(fromPeg, toPeg);
    solveHanoi(numDisks - 1, fromPeg, sparePeg);
    //2) now that most bottom disk is exposed, move it to the target peg
    hanoi.moveDisk(fromPeg, toPeg);
    // 3) make another recursive call to move the remaining disk back on top of bottom disk from spare peg to target peg
